...
plt.title(f'Tumor Purity Distribution \n({test_name}: t = {stat:.2f}, p = {p_val:.3f})', pad=20)

plt.ylabel('Statescope malignant fraction estimate')
plt.xlabel('Group')
plt.ylim(0, 1.1)
...
# 4. Save Figure
#-------------------------------------------------------------------------------
#save_dir = "/net/beegfs/users/P086608/Statescope/StatescopePro_v2/TCGA_bulk/Output/IFNE_CDKN2AB"
save_dir = '/net/beegfs/users/P086608/CIBERSORT/data/TCGA/output/plots'

plt.savefig(os.path.join(save_dir, "purity_comparison_G3_G4.pdf"))
print(f"Purity boxplot saved to: {os.path.join(save_dir, 'purity_comparison_G3_G4.pdf')}")
