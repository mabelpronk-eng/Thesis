...
def Get_file_paths(Sample_dict, data_dir):
    ...
    for project,id_dict in Sample_dict.items():
        ...
        for id in id_dict:
            # glob the filename
            path = glob.glob(f"{data_dir}{project}/{project}/Copy_Number_Variation/Masked_Copy_Number_Segment/{id}/*.txt")[0]
            paths.append(path)
    return paths
...
def Migrate_files(Sample_dict,paths, data_dir):
    ...
    :returns: list paths: list of paths to input files
    """
    dest_dir = os.path.join(data_dir, "copynumber", "segments")
    os.makedirs(dest_dir, exist_ok=True)  # Create directory if it doesn't exist

    for path in paths:
        project = list(Sample_dict.keys())[0]
        id = path.split("/")[-2]
