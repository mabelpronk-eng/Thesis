...
if(exists("snakemake")){
    ...
    ACE_fits <- Sys.glob("/net/beegfs/users/P086608/bulkRNA_glioma/data/GBM/copynumber/ACE/*/**/fits.txt")
    Purities <-  "/net/beegfs/users/P086608/bulkRNA_glioma/data/GBM/purity/GBM_Tumor_purities.tsv"
    Purities_out <- "/net/beegfs/users/P086608/bulkRNA_glioma/data/GBM/purity/ACE_Tumor_purities_TCGA-LUAD.tsv"
    Scatterplots_out <- "/net/beegfs/users/P086608/bulkRNA_glioma/data/GBM/copynumber/ACE/TCGA-95-7039/2N/errorgraph.svg"
}
#-------------------------------------------------------------------------------
# 1.1 Read data
#-------------------------------------------------------------------------------
...
ACE_purities <-
    tibble::tribble(
                ~sample,~ACE,
                purrr::map_chr(ACE_fits,~strsplit(.x,"/")[[1]][11]), purrr::map_dbl(ACE_fits, ~read.table(.x,header=T)[1,2])) %>%
