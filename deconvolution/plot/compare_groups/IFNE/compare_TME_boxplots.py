...
df_melted = df_micro.reset_index().melt(
    id_vars=['Sample_ID', 'Group'], #if use CIBERSORT output make index > Sample_ID if Statescope use index
...
plt.title(f'Microenvironment Composition: {g1_name} vs {g2_name}')
...
save_name = f"boxplot_TME_{g1_name.replace(' ', '')}_vs_{g2_name.replace(' ', '')}.pdf"
